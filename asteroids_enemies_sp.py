...
version = '2026.09.06'
def random_avoid_range(min_val, max_val, avoid_range):
    val = random.uniform(min_val, max_val)
    if val < 0:# Negative
        if val > -avoid_range:# Inside Negative Avoid Range
            val = random.uniform(-avoid_range, min_val)
    elif val >= 0:# Positive 
         if val < avoid_range:# Inside Positive Avoid Range
            val = random.uniform(avoid_range, max_val)
    return val
bullets = []
Enemies = []
Asteroids = []
...
class Interface():
    def __init__(self):
        ...
        self.num_targets = 0
        self.exit_button = Button(text='Salir', scale_x=0.08, scale_y=0.03, position=(0.85, 0.485), color="#00ffff", text_size=1.0, text_color=color.black, eternal=True)
        self.score_lbl = Text(text="Puntuación", x=0.81, y=0.40, origin=(0, 0), scale=1.5, color=color.cyan, text_size=0.7, eternal=True)
        self.score_txt = Text(text="0.0", x=0.81, y=0.37, origin=(0, 0), scale=1.5, color=color.cyan, text_size=0.7, eternal=True)
        ...
        self.asteroids_lbl = Text(text="Número de Asteroides", x=0.325, y=0.49, origin=(0, 0), color=color.cyan, text_size=0.7, eternal=True)
        ...
        self.enemy_lbl = Text(text="Número de Enemigos", x=0.11, y=0.49, origin=(0, 0), color=color.cyan, text_size=0.7, eternal=True)
        ...
        self.start_stop_button = Button(text='Comenzar', scale_x=0.085, scale_y=0.025, position=(-0.05, 0.485), color=color.cyan, 
    ...
    def toggle_run_pause(self, key, arg=None):
        if key == 'escape' or arg == "toggle":
            if self.game_over.text == 'Juego Terminado':
                ...
                Enemies.clear()
                Asteroids.clear()
                self.start_stop_button.text = 'Comenzar'
                self.score_txt.text = '0'
                self.num_targets = 0        
                init_game()
            if application.paused:
                self.start_stop_button.text = 'Pausa'
...
class Asteroid(Entity):
    def __init__(self):
        ...
        position_x = random_avoid_range(-half_scale_width, half_scale_width, avoid_x)
        position_y = random_avoid_range(-half_scale_height, half_scale_height, avoid_y)  
        ...
        if not application.paused:    
            ...
            if self.intersects(PlayerShip).hit:
                ...
                if abs(distance(PlayerShip, self)) < ship_radius + asteroid_radius:
                    ...
                    interface.game_over.text = "Juego Terminado"
                    interface.start_stop_button.text = "Comenzar"
                    application.pause()        
    ...
    def update(self):
        if not application.paused:    
            ...
            if held_keys['up arrow']:# Forwards Thrust
                ...
                self.x += math.sin(angle_rad) * 0.05 * self.speed
                self.y += math.cos(angle_rad) * 0.05 * self.speed
...
class Enemy_Bullets(Entity):
    def __init__(self, position, direction, speed=8, life_time=1.5):
        super().__init__(model='sphere', position=position, scale=(0.05, 0.05, 0.05), collider='sphere', color=color.yellow)
        self.direction = direction.normalized()
        self.speed = speed
        self.life_time = life_time
class Enemy_Ships(Entity):
    def __init__(self, guns):
        ...
        self.enemy_bullets = []
        self.fire_delay = 0.05
        self.last_time_fired = 0.05
    ...
    def wrap(self):
        ...
        if self.y < -half_scale_height: self.y = half_scale_height
    def shoot(self):
        if self.last_time_fired >= self.fire_delay:
            for gun in self.guns:
...
def change_screen_size(default_size):
    msg1="¿Quieres Cambiar el Tamaño de la Pantalla?\n"
    msg2=f"El Tamaño Predeterminado es Pantalla Completa (Ancho = {default_size}).\n"
    msg3="Si es así, Ingresa el Nuevo Ancho de Pantalla Abajo y Luego Selecciona OK.\n"
    msg4="La Altura de la Pantalla se Calculará Usando el Nuevo Ancho.\n"
    msg5="Si No, Selecciona Cancel."
